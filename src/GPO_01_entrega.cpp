...
void main()
 {
  ...
  col = 1-color  ;                         // Paso color a fragment shader
 }
);
...
vec3 up = vec3(0,0,1); // Hacia arriba
//vec3 up = vec3(0,1,1); // En diagonal 

float fov = 40.0f, aspect = 4.0f / 3.0f; //###float fov = 40.0f, aspect = 4.0f / 3.0f;
...
void render_scene()
{
	...
	// CODIGO AÑADIDO // EJ:2.1 : up antes valia : vec3 up = vec3(0,0,1);
	//V = lookAt(pos_obs, target, vec3(0,1,1));
	//
...
int main(int argc, char* argv[])
{
	...
	// Codigo añadido 
	glfwSwapInterval(0); // Anular refresco de pantalla preestablecido -Ej1.3
	// Comentar la siguiente liena para que funcione
...
void ResizeCallback(GLFWwindow* window, int width, int height)
{
	...
	// CODIGO AÑADIDO //
	aspect = (float)ANCHO / (float)ALTO; // Aspecto proporcional al tamanyo de la ventana - Ej1.1
	fov = (40.0f / 600.0f) * ALTO; // Campo visual proporcional al alto - Ej1.2
	//
}
...
static void KeyCallback(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	if (action == GLFW_PRESS || action == GLFW_REPEAT) 
    {
		...
		printf("Posición del observador: (%f, %f, %f)\n", pos_obs.x, pos_obs.y, pos_obs.z);
    }
	//
}
