...
objeto crear_cuadrado(void)
{
	...
	static const GLfloat vertex_data[] = {   //  POsicion XYZ      (u,v)
											-1.0f,  1.0f, 0.0f,   -1,1,  // Vertice 0
											 1.0f,  1.0f, 0.0f,    1,1,  // Vertice 1
											 1.0f, -1.0f, 0.0f,    1,-1,  // Vertice 2
											-1.0f, -1.0f, 0.0f,   -1,-1}; // Vertice 3
	

	GLbyte indices[] = { 0, 3, 2, 
						0, 2, 1 };
	obj.Ni = 2 * 3;
...
void render_scene()
{
	...
	float angle = t * 5.0f; // 5 grados por segundo

	float esc[4] = {1.0f, 0.27f, 0.53f, 11.0f};
	float scale = esc[counter % 4];

	///////// Aqui vendr�a nuestr c�digo para actualizar escena  /////////	
	mat4 M, T, R, S;
	R = rotate(radians(angle), vec3(0.0f, 0.0f, 1.0f)); // Matriz de rotación
	S = glm::scale(glm::mat4(1.0f), vec3(scale)); // Escalado uniforme       
