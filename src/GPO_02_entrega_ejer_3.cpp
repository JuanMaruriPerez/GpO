...
objeto crear_octaedro(void) {
    ...
    GLfloat vertex_data[] =
            {       // posicion vertice  // color vertice
                ...
                0.0f,  1.0f, 0.0f, 	1.00f, 0.00f, 0.00f,   // v2 
               -1.0f,  0.0f, 0.0f,  0.00f, 1.00f, 1.00f,   // v3  
                0.0f, -1.0f, 0.0f, 	0.00f, 1.00f, 0.00f,   // v4 
                0.0f,  0.0f,-1.0f,  1.00f, 0.00f, 1.00f,   // v5 
            };
...
void init_scene() {
    obj1 = crear_octaedro();  // Datos del objeto, mandar a GPU
    obj2 = crear_cubo();
...
void render_scene() {
    ...
    (!invertirObjs) ? dibujar_indexado(obj2) : dibujar_indexado(obj1);
	...
	M = T * R * S;
	transfer_mat4("MVP", Proy * View * M);
    (!invertirObjs) ? dibujar_indexado(obj1) : dibujar_indexado(obj2);
}
    ...
    switch (key) {
        ...
        case GLFW_KEY_F1:
            if (key == GLFW_KEY_F1 && action == GLFW_PRESS) {  // Solo al pulsar (no al liberar)
                invertirObjs = !invertirObjs;      // Invertir el orden
            }
    }
}
