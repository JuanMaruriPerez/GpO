...
const char* tcs_shader = GLSL(
    ...
    out vec2 vUV_TCS[];    // Definir como salida para pasar al TES

    uniform float tessellationFactor;

    void main() {
        gl_out[gl_InvocationID].gl_Position = gl_in[gl_InvocationID].gl_Position;
        vUV_TCS[gl_InvocationID] = vUV[gl_InvocationID];  // Pasar UV a TES
...
void render_gui() {
    ...
    ImGui::SliderFloat("Outline Factor", &heightFactor, 1.0f, 128.0f);
    ImGui::End();
...
void init_scene()
{
    ...
    //textura_id = cargar_textura("./data/Tronco/Rock030_4K-JPG_Color.jpg", GL_TEXTURE0);
    ...
    glUniform1f(glGetUniformLocation(prog, "heightFactor"), heightFactor); 
...
static void KeyCallback(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	if (action == GLFW_PRESS || action == GLFW_REPEAT) {  // Solo hacer algo si la tecla es presionada
        ...
        if (action == GLFW_PRESS || action == GLFW_REPEAT) {
            ...
            std::cout << "Height factor: " << heightFactor << std::endl;
        }
